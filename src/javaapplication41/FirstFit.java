 ...
 * @author Eslam
 */
public class FirstFit extends JPanel {
    ...
    /*This is an algorithm that returns the firstfit for each process*/
    public int EarlierFit(ArrayList<Integer> Hsizes, ArrayList<Integer> Psizes,int n){
    ...
    /*This function changes the position of each process according
    to the hole it's allocated to
    */
    public void FFit(ArrayList<Integer> Hsizes,int n)
    {       /*reset position*/
            y3=115;
            Pi.add(new JLabel());
            int First = EarlierFit(L1,L2,n);
            if(First !=-1){
             for(int r=0;r<HoleNo;r++)
             {
                 if(Hsizes.get(r) == Hsizes.get(First) )
                  break;
                 else
                  y3=y3+60;
             }
